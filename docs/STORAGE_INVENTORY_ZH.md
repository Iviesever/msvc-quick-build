...
这些是**成功调用的关联，不是物理输出观察或生命周期授权**。本轮不增加文件身份／内容哈希、并发写者排斥、当前内容核验、崩溃恢复、磁盘序列化或保留策略。旧缓存字节格式不变。历史成功仅作为调用方拥有的数据保存，之后的配置覆盖或失败构建可能改变实际文件。旧文件和 storage 的存活／可回收字段仍保持未知。模块／静态目标接入、PCH 生产者记录、持久化与读回另行推进，不能提前清理。默认 CLI 没有调用这一接口。
