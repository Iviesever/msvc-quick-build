...
这些是**成功调用的关联，不是物理输出观察或生命周期授权**。本轮不增加文件身份／内容哈希、并发写者排斥、当前内容核验、崩溃恢复、磁盘序列化或保留策略。旧缓存字节格式不变。历史成功仅作为调用方拥有的数据保存，之后的配置覆盖或失败构建可能改变实际文件。旧文件和 storage 的存活／可回收字段仍保持未知。静态目标支持见下节；模块接入、PCH 生产者记录、持久化与读回仍需另行推进，不能提前清理。默认 CLI 没有调用这一接口。
...
**归档配方身份不等于编译配置或文件内容身份。** Debug／Release 可能覆盖同一路径的对象和库，却保持同一个归档配方签名；必须另看编译选项及原始编译结果。调用方代际标签不保证唯一，缓存命中也不是新产生的库代际。模块与 PCH 生产者、物理文件身份、并发写入边界、持久化／读回及 Rium 验证仍未完成。本轮不引入清理、事务／租约、保留策略或 storage JSON 变更。
