...
Cumulative Entry Validation pins the released v5.5.0 source, complete candidate `cfb77425`, and a separate harness. Windows/Linux preflight preserves all three source archives, parent lineage, and inherited HOLDs; `preflight-completed.json` is not a performance pass. Issue #164 comment5740142076 allocates only PR #188's synchronization from `3196245`, workflow number5/attempt1, with matching actual head/base. Both preflights must pass before the single 584-pair / 1,446-invocation comparison. Other events, repeats, and manual requests have no measurement allocation. Failures and unrun samples remain; no refill. This product candidate and recorder genuinely differ from the old experiment, but a new green cannot clear old risks or establish their causes.

The original counters/statistics, timings-OFF scores, audits, and first-failure rules remain. Archive sources before building and retain the actual seed and A/B programs with before/after identities. The existing twelve product execution controls now retain raw output and exit codes. Invocation and program-control counts have pre-dispatch limits. Keep the accepted append journal and compatible final JSON on controlled exits. Publication still requires individual risk disposition and independent VERSION/release acceptance.
