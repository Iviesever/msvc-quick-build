...
| C — Safe/conditional passthrough | Does not invalidate MQB structural ownership and can remain an ordinary MSVC argv element | `/W4`, `/WX`, `/fp:fast`, `/arch:AVX2`, `/favor:AMD64`, `/I`, `/D`, `/U`, `/external:I`, `/STACK`, version-conditional `/DEBUG:FASTLINK` | Preserve native argv ownership/order; existing signatures/cache identity include it, then apply toolchain lifecycle admission where required |
| D — Unsupported / conflicting | Changes an unmodeled pipeline, hides inputs, introduces untracked files/artifacts, is unconditionally obsolete, or conflicts with another semantic value | `/MP`, raw PCH `/Y*`, `/FI`, `/FU`, `/analyze`, `/experimental:log`, `@response` | Fail closed before invoking MSVC |
...
This distinction is spelling-sensitive. Attached forms such as `/Iinclude` or `/external:Ivendor` consume no additional token. Exact split forms such as `/I include`, `/D NAME`, `/U NAME`, `/external:I vendor`, `/Tc file`, `/Tp file`, `/headerName:angle vector`, and `/ifcOutput path` consume one token. A split operand is grouped even when the option is later rejected; for example `/FI forced.hpp`, `/analyze:plugin checker.dll`, and `/experimental:log sarif-out` reach the Parameter Engine as intact option/operand pairs instead of misclassifying the pathname as a source.
...
Raw forced-include `/FI` is rejected even though its split token shape is understood: it changes every TU's effective include graph, and MQB has not yet represented that forced edge in smart discovery/freshness evidence. Raw `/FU` is rejected because it introduces a metadata file input that is absent from compile freshness identity.
...
`cpp/tests/app/cli/cli_argument_tests.cpp` independently verifies the CLI boundary: fixed operands stay in ordered compiler argv, attached forms do not consume the next source, and even later-rejected options reach Parameter Engine routing with their operands intact.
...
- raw forced include/metadata (`/FI`, `/FU`): rejected until their dependency inputs participate in discovery/freshness identity;
