...
取消或正常根进程退出时，终止 Job 并查询 active process accounting 直至为零，再完成
管道 reader 的 join 后返回。只等根进程或等 Job 句柄变为 signaled 都不足以建立该边界。
...
根先退出而后代存活、同 runner 并发取消隔离、owner 被终止，以及重复调用句柄计数。
