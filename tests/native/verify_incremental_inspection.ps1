...
$contractSource = Join-Path $RepoRoot 'tests/native/incremental_inspection_contract.cpp'
foreach ($path in @($configPath, $contractSource)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Incremental inspection evidence input missing: $path"
    }
}
...
if ($productionSources.Count -eq 0) {
    throw 'cpp/mqb.json has no production sources for inspection evidence.'
}

$arguments = [System.Collections.ArrayList]::new()
[void]$arguments.Add('tests/native/incremental_inspection_contract.cpp')
...
[void]$arguments.Add('incremental_inspection_contract')
...
try {
    ...
    if ($buildExit -ne 0) {
        throw "Failed to build incremental inspection contract (exit $buildExit)."
    }

    $contractExe = Join-Path $RepoRoot '.mqb/bin/incremental_inspection_contract.exe'
    if (-not (Test-Path -LiteralPath $contractExe -PathType Leaf)) {
        throw "Inspection contract executable missing: $contractExe"
    }
    ...
    if ($testExit -ne 0) {
        throw "Incremental inspection contract failed (exit $testExit)."
    }
}
finally {
    Pop-Location
...
Write-Host 'Incremental inspection evidence passed.'
