...
$contractSource = Join-Path $RepoRoot 'tests/native/module_compile_inspection_contract.cpp'
foreach ($path in @($configPath, $contractSource)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Module compile inspection evidence input missing: $path"
    }
}
...
if ($productionSources.Count -eq 0) {
    throw 'cpp/mqb.json has no production sources for module compile inspection evidence.'
}

$arguments = [System.Collections.ArrayList]::new()
[void]$arguments.Add('tests/native/module_compile_inspection_contract.cpp')
...
[void]$arguments.Add('module_compile_inspection_contract')
...
try {
    ...
    if ($buildExit -ne 0) {
        throw "Failed to build module compile inspection contract (exit $buildExit)."
    }

    $contractExe = Join-Path $RepoRoot '.mqb/bin/module_compile_inspection_contract.exe'
    if (-not (Test-Path -LiteralPath $contractExe -PathType Leaf)) {
        throw "Module compile inspection contract executable missing: $contractExe"
    }
    ...
    if ($testExit -ne 0) {
        throw "Module compile inspection contract failed (exit $testExit)."
    }
}
finally {
    Pop-Location
...
Write-Host 'Module compile inspection evidence passed.'
