...
if ($allTestFiles.Count -ne 84) {
    throw "Native test manifest drift: expected 84 *_tests.cpp files, found $($allTestFiles.Count)."
}
...
foreach ($weightedPath in $testWeightOverrides.Keys) {
    if ($weightedPath -notin $relativeTestPaths) {
        throw "Native test weight override is stale or missing from the 84-test manifest: $weightedPath"
    }
}
