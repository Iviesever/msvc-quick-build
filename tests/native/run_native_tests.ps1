...
if ($allTestFiles.Count -ne 86) {
    throw "Native test manifest drift: expected 86 *_tests.cpp files, found $($allTestFiles.Count)."
}
...
foreach ($weightedPath in $testWeightOverrides.Keys) {
    if ($weightedPath -notin $relativeTestPaths) {
        throw "Native test weight override is stale or missing from the 86-test manifest: $weightedPath"
    }
}
