...
if ($allTestFiles.Count -ne 82) {
    throw "Native test manifest drift: expected 82 *_tests.cpp files, found $($allTestFiles.Count)."
}
...
foreach ($weightedPath in $testWeightOverrides.Keys) {
    if ($weightedPath -notin $relativeTestPaths) {
        throw "Native test weight override is stale or missing from the 82-test manifest: $weightedPath"
    }
}
