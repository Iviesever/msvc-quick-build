...
if ($allTestFiles.Count -ne 88) {
    throw "Native test manifest drift: expected 88 *_tests.cpp files, found $($allTestFiles.Count)."
}
...
foreach ($weightedPath in $testWeightOverrides.Keys) {
    if ($weightedPath -notin $relativeTestPaths) {
        throw "Native test weight override is stale or missing from the 88-test manifest: $weightedPath"
    }
}
