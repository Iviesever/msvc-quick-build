...
if ($allTestFiles.Count -ne 81) {
    throw "Native test manifest drift: expected 81 *_tests.cpp files, found $($allTestFiles.Count)."
}
...
foreach ($weightedPath in $testWeightOverrides.Keys) {
    if ($weightedPath -notin $relativeTestPaths) {
        throw "Native test weight override is stale or missing from the 81-test manifest: $weightedPath"
    }
}
