...
if ($allTestFiles.Count -ne 83) {
    throw "Native test manifest drift: expected 83 *_tests.cpp files, found $($allTestFiles.Count)."
}
...
foreach ($weightedPath in $testWeightOverrides.Keys) {
    if ($weightedPath -notin $relativeTestPaths) {
        throw "Native test weight override is stale or missing from the 83-test manifest: $weightedPath"
    }
}
