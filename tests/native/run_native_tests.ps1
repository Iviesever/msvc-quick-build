...
if ($allTestFiles.Count -ne 85) {
    throw "Native test manifest drift: expected 85 *_tests.cpp files, found $($allTestFiles.Count)."
}
...
foreach ($weightedPath in $testWeightOverrides.Keys) {
    if ($weightedPath -notin $relativeTestPaths) {
        throw "Native test weight override is stale or missing from the 85-test manifest: $weightedPath"
    }
}
