...
if ($allTestFiles.Count -ne 87) {
    throw "Native test manifest drift: expected 87 *_tests.cpp files, found $($allTestFiles.Count)."
}
...
foreach ($weightedPath in $testWeightOverrides.Keys) {
    if ($weightedPath -notin $relativeTestPaths) {
        throw "Native test weight override is stale or missing from the 87-test manifest: $weightedPath"
    }
}
