...
function Assert-ExactFiles {
    ...
    $actual = @(Get-ChildItem -LiteralPath $Root -File | Select-Object -ExpandProperty Name | Sort-Object -Unique)
    $expectedSorted = @($Expected | Sort-Object -Unique)
    $diff = @(Compare-Object -ReferenceObject $expectedSorted -DifferenceObject $actual)
    if ($diff.Count -ne 0) {
        $details = @($diff | ForEach-Object {
...
if (-not (Test-Path -LiteralPath $CppRoot -PathType Container)) { throw "C++ source root not found: $CppRoot" }
...
foreach ($root in @($includeRoot, $srcRoot, $testsRoot)) {
    if (-not (Test-Path -LiteralPath $root -PathType Container)) { throw "Required C++ responsibility root not found: $root" }
}

Assert-DirectDirectories -Root $includeRoot -Allowed @('config', 'core', 'discovery', 'json', 'modules', 'msvc', 'orchestration', 'platform', 'process')
Assert-DirectDirectories -Root $srcRoot -Allowed @('app', 'config', 'core', 'discovery', 'json', 'modules', 'msvc', 'orchestration', 'platform')
Assert-DirectDirectories -Root $testsRoot -Allowed @('app', 'config', 'core', 'discovery', 'e2e', 'json', 'modules', 'msvc', 'orchestration', 'platform', 'process')
...
foreach ($leaf in $appLeafFiles.Keys) {
    ...
    if (-not (Test-Path -LiteralPath $leafRoot -PathType Container)) { throw "Required app responsibility directory not found: $leafRoot" }
    Assert-ExactFiles -Root $leafRoot -Expected $appLeafFiles[$leaf]
}
Assert-LeafLayout -Root (Join-Path $testsRoot 'app') -LeafFiles ([ordered]@{'cli' = @('build_policy_cli_tests.cpp', 'cli_argument_tests.cpp')})
...
foreach ($leaf in $discoveryLeafFiles.Keys) {
    ...
    if (-not (Test-Path -LiteralPath $leafRoot -PathType Container)) { throw "Required discovery responsibility directory not found: $leafRoot" }
...
Assert-ExactFiles -Root $discoveryTestsRoot -Expected @('c_source_discovery_tests.cpp','module_source_discovery_tests.cpp','module_syntax_tests.cpp','source_discovery_corrections_tests.cpp','source_discovery_tests.cpp')

$coreLeafFiles = [ordered]@{
    'cache' = @('ArchiveCache.cpp','ArchiveCacheFile.cpp','CompileCache.cpp','CompileCacheFile.cpp','LinkCache.cpp','LinkCacheFile.cpp')
...
Assert-LeafLayout -Root (Join-Path $testsRoot 'core') -LeafFiles ([ordered]@{
    'cache' = @('compile_cache_file_tests.cpp','compile_cache_tests.cpp','link_cache_file_tests.cpp','link_state_tests.cpp')
    'model' = @('build_request_tests.cpp','build_signature_tests.cpp','c_compile_signature_tests.cpp','c_source_recipe_tests.cpp','translation_unit_classifier_tests.cpp')
    'planning' = @('build_plan_tests.cpp','build_planner_tests.cpp','dependency_graph_tests.cpp','project_artifact_layout_tests.cpp')
})

$msvcLeafFiles = [ordered]@{
    'compiler' = @('CompilerArgumentBuilder.cpp','CompilerArgumentBuilder.hpp','CompilerExecution.cpp','CompilerExecution.hpp','CompilerInvocationValidation.cpp','CompilerInvocationValidation.hpp','MsvcCompileExecutor.cpp','MsvcCompiler.cpp','MsvcSourceDependenciesReader.cpp')
    'librarian' = @('MsvcLibrarian.cpp')
    'linker' = @('MsvcLibraryResolver.cpp','MsvcLinker.cpp')
    'modules' = @('MsvcModuleDependencyScanner.cpp')
    'parameters' = @('MsvcParameterEngine.cpp','MsvcParameterRegistry.cpp','MsvcParameterRegistry.hpp')
    'toolchain' = @('MsvcToolchainLocator.cpp','PortableToolchainDiscovery.cpp','PortableToolchainDiscovery.hpp','ToolchainDiscoveryPrimitives.cpp','ToolchainDiscoveryPrimitives.hpp','VisualStudioEnvironment.cpp','VisualStudioEnvironment.hpp','VisualStudioToolchainDiscovery.cpp','VisualStudioToolchainDiscovery.hpp')
}
...
Assert-LeafLayout -Root (Join-Path $testsRoot 'msvc') -LeafFiles ([ordered]@{
    'compiler' = @('compile_executor_tests.cpp','compile_integration_tests.cpp','compiler_arguments_tests.cpp','incremental_loop_tests.cpp','source_dependencies_tests.cpp')
    ...
    'modules' = @('header_unit_compile_integration_tests.cpp','header_unit_compiler_tests.cpp','module_compile_integration_tests.cpp','module_dependency_scanner_integration_tests.cpp','module_dependency_scanner_tests.cpp')
...
$orchestrationLeafFiles = [ordered]@{
    'incremental' = @('IncrementalFileSnapshot.hpp','MsvcIncrementalArchiveCoordinator.cpp','MsvcIncrementalCompileCoordinator.cpp','MsvcIncrementalLinkCoordinator.cpp','MsvcIncrementalStaticTargetCoordinator.cpp','MsvcIncrementalTargetCoordinator.cpp')
    'modules' = @('ModuleCompilePlan.cpp','ModuleCompilePlan.hpp','ModuleCompileRequestFactory.cpp','ModuleCompileRequestFactory.hpp','ModuleTargetArtifactRegistry.cpp','ModuleTargetArtifactRegistry.hpp','ModuleTargetLinkRequestFactory.cpp','ModuleTargetLinkRequestFactory.hpp','ModuleTargetPreparation.cpp','ModuleTargetPreparation.hpp','ModuleTargetScanner.cpp','ModuleTargetScanner.hpp','MsvcModuleCompileCoordinator.cpp','MsvcModuleTargetCoordinator.cpp','StandardLibraryModuleProvider.cpp','StandardLibraryModuleProvider.hpp')
...
Assert-LeafLayout -Root (Join-Path $testsRoot 'orchestration') -LeafFiles ([ordered]@{
    'incremental' = @('incremental_archive_coordinator_tests.cpp','incremental_compile_coordinator_tests.cpp','incremental_link_coordinator_tests.cpp','incremental_target_coordinator_tests.cpp')
    'modules' = @('header_unit_incremental_integration_tests.cpp','header_unit_target_integration_tests.cpp','header_unit_wave_integration_tests.cpp','module_compile_coordinator_tests.cpp','module_target_coordinator_tests.cpp','module_target_integration_tests.cpp','module_target_validation_tests.cpp')
