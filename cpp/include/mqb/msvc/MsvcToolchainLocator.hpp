...
struct DiscoveryOptions {
    ...
    // Visual Studio discovery only. nullopt delegates the default to the VS
    // cache authority, which selects an architecture-specific file below the
    // current working directory's .mqb tree. The MQB application replaces this
    // with the active project root's explicit architecture-specific .mqb cache
    // path. An explicitly empty path disables reuse. Custom vswhere/cmd
