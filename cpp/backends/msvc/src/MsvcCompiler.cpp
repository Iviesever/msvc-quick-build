...
[[nodiscard]] CompilerError invalid_request(std::string message) {
    return CompilerError{.code = CompilerErrorCode::invalid_request, .message = std::move(message)};
}
...
[[nodiscard]] std::expected<void, CompilerError> append_common_compile_arguments(
    ...
    if (options.runtime_library) {
        // The explicit typed runtime is emitted after the preset so it is the
        // authoritative MSVC runtime switch without changing default recipes.
    ...
    for (const auto& define : options.defines) {
        if (define.empty()) return std::unexpected(invalid_request("compiler define must not be empty"));
        arguments.push_back("/D" + define);
    }
    for (const auto& include_directory : options.include_directories) {
        if (include_directory.empty()) return std::unexpected(invalid_request("include directory must not be empty"));
        arguments.push_back("/I" + path_to_utf8(include_directory));
    }
    for (const auto& argument : options.additional_arguments) {
        if (argument.empty()) return std::unexpected(invalid_request("additional compiler argument must not be empty"));
...
[[nodiscard]] std::string header_unit_key(const HeaderUnitReference& reference) {
    return std::string{reference.lookup_method == HeaderUnitLookupMethod::angle ? "angle:" : "quote:"}
...
[[nodiscard]] std::expected<void, CompilerError> validate_module_contract(
    ...
    if (uses_module_contract && predates_cpp20(invocation.options.standard)) {
        return std::unexpected(invalid_request("module and header-unit compilation requires C++20 or newer"));
    }
    if (invocation.kind == TranslationUnitKind::module_interface) {
        ...
        return std::unexpected(invalid_request("ordinary source compilation must not request an IFC output"));
    }
    ...
    for (const auto& reference : invocation.module_references) {
        if (reference.logical_name.empty()) return std::unexpected(invalid_request("module reference logical name must not be empty"));
        if (reference.interface_file.empty()) return std::unexpected(invalid_request("module reference IFC path must not be empty"));
        if (!logical_names.emplace(reference.logical_name).second) {
            return std::unexpected(invalid_request("duplicate module reference logical name '" + reference.logical_name + "'"));
        }
    }
    ...
    for (const auto& reference : invocation.header_unit_references) {
        if (reference.header_name.empty()) return std::unexpected(invalid_request("header-unit reference name must not be empty"));
        if (reference.interface_file.empty()) return std::unexpected(invalid_request("header-unit reference IFC path must not be empty"));
        const std::string key = header_unit_key(reference);
        if (!header_names.emplace(key).second) {
            return std::unexpected(invalid_request("duplicate header-unit reference '" + reference.header_name + "'"));
        }
    }
    return {};
}
...
MsvcCompiler::build_arguments(const CompileInvocation& invocation) {
    if (invocation.source.empty()) return std::unexpected(invalid_request("compile source path is empty"));
    if (invocation.object.empty()) return std::unexpected(invalid_request("compile object path is empty"));
    ...
    if (!module_contract) return std::unexpected(module_contract.error());

    const bool c_translation_unit = is_c_translation_unit_path(invocation.source);
    std::vector<std::string> arguments;
    arguments.reserve(22 + invocation.options.defines.size()
    ...
    auto common = append_common_compile_arguments(arguments, invocation.options, c_translation_unit);
...
MsvcCompiler::build_header_unit_arguments(const HeaderUnitCompileInvocation& invocation) {
    if (invocation.header_name.empty()) return std::unexpected(invalid_request("header-unit name must not be empty"));
    if (invocation.interface_output.empty()) return std::unexpected(invalid_request("header-unit IFC output path must not be empty"));
    if (invocation.object && invocation.object->empty()) return std::unexpected(invalid_request("header-unit object output path must not be empty"));
    if (invocation.source_dependencies && invocation.source_dependencies->empty()) return std::unexpected(invalid_request("sourceDependencies output path is empty"));
    if (predates_cpp20(invocation.options.standard)) return std::unexpected(invalid_request("header-unit compilation requires C++20 or newer"));

    std::vector<std::string> arguments;
    arguments.reserve(23 + invocation.options.defines.size()
        + invocation.options.include_directories.size()
        + invocation.options.additional_arguments.size());
...
MsvcCompiler::compile(const CompileInvocation& invocation) const {
    ...
    if (!prepared_object) return std::unexpected(prepared_object.error());
    if (invocation.source_dependencies) {
        auto prepared_dependencies = prepare_parent_directory(*invocation.source_dependencies, "sourceDependencies");
        if (!prepared_dependencies) return std::unexpected(prepared_dependencies.error());
    }
    if (invocation.module_interface_output) {
        auto prepared_interface = prepare_parent_directory(*invocation.module_interface_output, "module interface");
        if (!prepared_interface) return std::unexpected(prepared_interface.error());
    }
